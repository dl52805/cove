  ...
  rules: {
    ...
    declaration: $ => seq(
      ...
      commaSep1(field('declarator', choice(
        seq(
          // C uses _declaration_declarator here for some nice macro parsing in function
          // declarators, but this causes a world of pain for C++ so we'll just stick to
          // the normal _declarator here.
    ...
    meta_args: ($) =>
      seq(
        "(",
        commaSep(choice($.identifier, $.string_literal, $.assigned_arg)),
        ")",
      ),

    meta_tag: ($) =>
      seq(field("name", $.identifier), field("args", $.meta_args)),

    enum_specifier: $ => prec.right(seq(
      'enum',
      optional(choice('class', 'struct')),
      optional(field("attr", $.attribute_declaration)),
      choice(
        seq(
          field('name', $._class_name),
          optional($._enum_base_clause),
          optional(field('body', $.meta_enumerator_list)),
        ),
        field('body', $.meta_enumerator_list),
      ),
      optional($.attribute_specifier),
    )),
